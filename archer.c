    ...
    if (!(ent->state == ENTITY_STATE_ATTACK_FAILING) && drawable_entity_get_frame(ent->drawable) == 12){
        switch (*current_state){
            case ARCHER_SHORT_HIT:
                ...
                break;
            case ARCHER_CRIT:
                battle_add_object(g,arrow_init((int)ent->pos,(unsigned int)stats->range, (unsigned int)stats->arrow_speed,stats->arrow_size, ent->team, ent->facing, stats->critical_damage));
                break;
            case ARCHER_NORMAL:
